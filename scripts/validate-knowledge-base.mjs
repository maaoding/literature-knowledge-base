...
const timelineSource = fs.readFileSync(path.join(docsDir, '.vitepress', 'theme', 'components', 'TimelineView.vue'), 'utf8')
const catalogNodeSource = fs.readFileSync(path.join(docsDir, '.vitepress', 'content', 'catalog.node.ts'), 'utf8')
...
assert(workExplorerSource.includes("const viewMode = ref<ViewMode>('cards')") && workExplorerSource.includes("viewMode.value !== 'cards'"), 'work index must use cards as its canonical default view')
assert(authorExplorerSource.includes("const viewMode = ref<ViewMode>('cards')") && authorExplorerSource.includes("viewMode.value !== 'cards'"), 'author index must use cards as its canonical default view')
assert(catalogNodeSource.includes("text: '文学史索引'") && catalogNodeSource.includes("text: '总览与时间线'"), 'history sidebar must expose an independent index group')
assert(!catalogNodeSource.includes("index === 0 ? [{ text: '总览'"), 'history overview must not be nested under Chinese literary history')
assert(timelineSource.includes('class="kb-timeline-row"') && !timelineSource.includes('kb-timeline-card'), 'history timeline must use compact chronological rows')
assert(!timelineSource.includes('kb-track-headings') && !timelineSource.includes("'is-single'"), 'history timeline must not restore fixed China/world columns')
...
const countClassToken = (html, className) => (html.match(/class="[^"]*"/g) ?? [])
  .filter((attribute) => attribute.slice(7, -1).split(/\s+/).includes(className))
  .length
...
assert(countClassToken(historyIndexBuild, 'kb-timeline-row') === 29, 'history index SSR must render exactly 29 timeline rows')
assert(!historyIndexBuild.includes('kb-timeline-card') && !historyIndexBuild.includes('kb-track-headings'), 'history index still renders the old two-track cards')
assert((workIndexBuild.match(/class="kb-card"/g) ?? []).length === 20, 'work index SSR must render exactly 20 cards')
assert(!workIndexBuild.includes('class="kb-catalog-row'), 'default work index SSR must not render list rows')
assert(!workIndexBuild.includes('class="kb-guide-row"'), 'default work index SSR must stay in catalog mode')
assert((authorIndexBuild.match(/class="kb-card"/g) ?? []).length === 20, 'author index SSR must render exactly 20 cards')
assert(!authorIndexBuild.includes('class="kb-catalog-row'), 'default author index SSR must not render list rows')
